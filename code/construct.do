   ...
   *** Characteristics sheet from user-written excel file **
  ...
  //Common steps for Type-1 and Type-2 vars
 if (`n2' > 0) {
  forv i = 1/`c(N)' {
	preserve
		keep in `i'/`i'
		...
		if (("`name'" != "CountryName") & ("`name'" != "WHO_Region2") & ///
			...
			("`name'" != "GNI") & ("`name'" != "ESncomponent") & ("`name'" != "strategy")) {
				...
				forv j = 1/`n3' {
					...
					keep if strpos(`name', "`: word `j' of `levs''") > 0
  ...
  replace user_input = subinstr(user_input," ","",.)
    ...
    if (`c(N)' > 0 & user_input != "") {
		...
		// Step 2: Destring and keep relevant rows for each level of var
		//keep strategy 
